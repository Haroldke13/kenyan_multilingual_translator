...
An offline-first Language to English speech translation service, with an installable
...
python -m language_ai.web.api --lan --https
```
...
than loading the models twice. Setting `LANGUAGE_API_URL` makes it forward uploads
to a FastAPI server instead of loading the models itself.

The same page also translates written text: type or paste Language and read the
...
Language is written with ĩ and ũ, but phone keyboards rarely offer them, so people
...
accented Language. Only about 1.6% of that vocabulary has more than one accented
...
🎤 Choose a recording and select the matching language (Language, Luo, Swahili, Somali, Oromo, Kamba)

## Languages

Language is the default. Pick another from the buttons at the top of the page, or
with `?lang=luo`. An unrecognised code falls back to Language rather than failing.

| Language | NLLB tag | MMS adapter | Bible verses |
| --- | --- | --- | --- |
| Language | kik_Latn | kik | 31,094 |
...
For Language there is no public labelled test set, so the two available models were
compared on 2 minutes of real Language audio by how many of the words they produce
are real Language, checked against the 53,620 word forms in the Bible corpus:

| | Kiragu whisper-small-language | MMS-1B (kik) |
| --- | --- | --- |
| Words that are real Language | 38.8% | **51.2%** |
| Words carrying tilde vowels | 11% | **46%** |
| Peak memory | **1.5 GB** | 4.5 GB |
| Time for 4 clips | **63 s** | 159 s |

MMS is the default for Language: it produces more real words and, importantly,
...
```sh
LANGUAGE_ASR_ENGINE=whisper python -m language_ai.web.flask_app --lan
```

This is a proxy measure, not a word error rate: it counts whether words exist in
Language, not whether they are the words that were said. Treat it as evidence that
...
loses all of it. `language_ai.clipper` cuts the recording into clips, records what
...
python -m language_ai.clipper sermon.mp4 --output data/clips/sermon --status
...
With `--translate`, each clip's Language and English are stored alongside it and
...
pip install -e '.[dev]'
python -m pytest
python -m language_ai.cli recording.mp3

# Optional HTTP API:
pip install -e '.[web]'
python -m language_ai.web.api
...
ffmpeg; every other format requires it. Configure `LANGUAGE_TTS_COMMAND` with a Piper-compatible command that accepts text on stdin and `--output_file PATH` if you want translated speech output.
...
```sh
pip install -e ".[web,ml]"
LANGUAGE_API_HOST=0.0.0.0 LANGUAGE_API_PORT=8000 python -m language_ai.web.api
```
...
```sh
export LANGUAGE_API_URL=http://SERVER_IP:8000
python -m language_ai.cli recording.mp3
python access_translator.py --audio recording.mp3
python -m ui.gradio_app
```

Text-only remote translation is available through `POST /translate-text` and `python access_translator.py --text "..."` when `LANGUAGE_API_URL` is set.
...
LANGUAGE_ASR_MODEL=Kiragu/whisper-small-language-v5
LANGUAGE_TRANSLATION_MODEL=nickdee96/nllb-200-600m-language-english
...
```sh
LANGUAGE_ASR_CHUNK_SECONDS=12
LANGUAGE_ASR_RETRY_CHUNK_SECONDS=6
LANGUAGE_ASR_SILENCE_RMS=0.001
LANGUAGE_ASR_MAX_NEW_TOKENS=96
LANGUAGE_ASR_NO_REPEAT_NGRAM_SIZE=3
LANGUAGE_ASR_REPETITION_PENALTY=1.15
LANGUAGE_TRANSLATION_MAX_NEW_TOKENS=128
LANGUAGE_TORCH_THREADS=1
LANGUAGE_KEEP_ASR_LOADED=0
LANGUAGE_KEEP_TRANSLATION_LOADED=0
```

For ASR verification without loading the larger translation model, run `python -m language_ai.cli --asr-only recording.mp3`.
...
download. The defaults are deliberately open-licensed: the Language text is Biblica's
Open Language edition under CC BY-SA 4.0, and the English is the World English Bible,
...
- **Recordings with sidecars** in `data/voice/`: put `clip01.wav` next to
  `clip01.language.txt` (what was said) and `clip01.english.txt` (what it means).
  These train both the translator and the speech recogniser.
- **Reviewed sessions**, marked by writing `language.corrected.txt` or
...
python training/train_translation.py data/translation/parallel.jsonl \
    --output models/translation/language-english
```
...
`LANGUAGE_TRANSLATION_MODEL=models/translation/language-english`.

## API

`GET /` returns the installable mobile app; the old status page with links to
`/health`, `/docs` and `/redoc` moved to `/status`. `POST /translate` with multipart field `file` returns the Language transcript, English result, confidence values, optional Bible match, and session file paths. `POST /corrections` stores human corrections in `data/corrections.db` for later fine-tuning.
...
```json
{"audio":"data/asr/train/clip001.wav","text":"Language transcript here","split":"train"}
{"audio":"data/asr/dev/clip101.wav","text":"Language transcript here","split":"eval"}
```
...
python training/train_asr.py data/asr/manifest.jsonl --output models/asr/language-whisper
```
...
```sh
LANGUAGE_ASR_MODEL=models/asr/language-whisper
```

## Translation fine-tuning

Create a licensed or corrected Language-English JSONL file:

```json
{"language":"Language source sentence","english":"English target sentence","split":"train"}
{"language":"Language eval sentence","english":"English eval sentence","split":"eval"}
```

Fine-tune the configured NLLB model:

```sh
python training/train_translation.py data/translation/parallel.jsonl --output models/translation/language-english
```
